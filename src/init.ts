...
async function get_pokemon(instance: DataManager) {
    const specie = await get_data_from<IPokemon>('pokemon.csv');
    const species = specie.reduce((acc: { [key: string]: IPokemon }, x) => {
        acc[x.id] = x;
        return acc;
    }, {});
    const evolutions = await get_data_from<IEvolution>('evolution.csv');
    const evolution = evolutions.reverse().reduce((acc: { [key: string]: IEvolution }, x) => {
